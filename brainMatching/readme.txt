BrainMatch is a program, that has two mjor uses: calculating brain network similarity of a subject relative to a group, and structure-function coupling of a subject. For the details of how to generate the executable and potential use cases, see information below.



1) Generating the executable brain matching program:
----------------------------------------------------
a) first, we need to extract the DLIB library files
	- go to lib folder:
		$> cd lib
	- and extract the tar ball:
		$> tar -zxvf dlib-19.4.tar.gz


b) Next, we compile the brainMatch code to generate the executable
	- In order to compile the brain mathing code, type:
		$> make -f makefile
	- This will generate Build and bin folders. Generated executable will be located under ./bin/ folder
	- If you are making changes to the code, you might want to remove previous builds. To do so, type:
		$> make -f makefile clean


2) Example codes
----------------
Although the brain matching program many things, two of its major uses are exemplified under ./examples/ folder
...
This use case of the code matches conenctome of a single subject with everybody else and in its evaluation, we consider average similarity of the individual relative to a group (such as healthycontrols). Thus, this use case of the code is groupwise matching.
