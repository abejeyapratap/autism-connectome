...
def heatMap(matrix,displayPlotFlag,outputPath,backgroundColor,threshold,colormap='jet'):
    ...
    plt.figure()
    plt.imshow(matrix, interpolation='nearest',vmin=threshold,cmap=cmap_)
    plt.colorbar()
    if(outputPath!=''):
...
def drawCorrelationPlot(data1,data2,r,p,data1Label,data2Label,plotTitle,outputPath,lineColor='darkorchid',dotColor='orchid',pointNames=None,text="",fontSize=16):
    import numpy as np
    import matplotlib.pylab as plt
    import matplotlib as mpl
    ...
    ax.set_xlabel(data1Label,fontsize=fontSize)
    ax.set_ylabel(data2Label,fontsize=fontSize)
    ax.tick_params(axis='both',labelsize=fontSize-5)
    mpl.rcParams['font.sans-serif'] = "Times New Roman"
    mpl.rcParams['font.family'] = "serif"
    # if(text==""):
    #     text='r='+str("{0:.2f}".format(r))+'\np='+str("{0:.3f}".format(p))
    # ax.text(0.79,0.9,text,horizontalalignment='left',verticalalignment='center',
    #         transform = ax.transAxes, color='grey',fontsize=fontSize/2,
    #         bbox={'facecolor':'white', 'alpha':0.5, 'pad':10})
    coeff=np.polyfit(data1,data2,1)
    data2Regressed=np.polyval(coeff,data1)
    plt.scatter(data1,data2,c=dotColor,edgecolors='none',alpha=0.7)
    
    if pointNames!=None:
        yTextOffset=(np.nanmax(data2)-np.nanmin(data2))/60.0 ## move the label of each node up in the y axis by this much
        xTextOffset=(np.nanmax(data1)-np.nanmin(data1))/60.0 ## move the label of each node up in the y axis by this much
        for i in range(len(pointNames)):
            ax.annotate(pointNames[i],(data1[i]-xTextOffset,data2[i]+yTextOffset),size=6)
    plt.plot(data1,data2Regressed,color=lineColor,linewidth=2)
    plt.title(plotTitle)
...
def drawCorrelationPlotWithConfidenceInterval(data1,data2,r,p,data1Label,data2Label,plotTitle,outputPath,pointNames=None,logScale=False):
    ...
    fig=plt.figure()
    ax=fig.add_subplot(111)
    ax.set_xlabel(data1Label,fontsize=16)
    ax.set_ylabel(data2Label,fontsize=16)
    ax.tick_params(axis='both',labelsize=10)
    
    mpl.rcParams['font.sans-serif'] = "Times New Roman"
    mpl.rcParams['font.family'] = "serif"
    ax.text(0.84,0.87, 'r='+str("{0:.2f}".format(r))+'\np='+("{0:.3f}".format(p)),
...
def drawHistogram(data,dataLabels,title,outputPath,xLabel="",yLabel="frequency",bins=32,colors=['#2096BA','#351C4D', '#AB3E16','#849974', '#F7DFD4','#F5AB99'],noBars=False,normalized=False): #shutter blue, nightfall, rust, fresh, macaron, tropical pink
    import numpy as np
    import scipy.stats as stt
    import matplotlib.pylab as plt
    import matplotlib as mpl
...
#            bbox={'facecolor':'white', 'alpha':0.5, 'pad':8})
    ...
    if(noBars==False):
        for i in range(numData):
            plt.hist(data[i], bins=bins, alpha=0.6,color=colors[i],edgecolor='none',label=dataLabels[i],density=normalized)
...
def drawHistogramSingleDataset(data,dataLabel,xLabel,yLabel,plotTitle,outputPath,color='dodgerblue',fontSize=16,xLim=[],yLim=[]):
    import numpy as np
    import scipy.stats as stt
    import matplotlib.pylab as plt
    import matplotlib as mpl
    ...
    if(len(xLim)==2):#use this line to set the y limits of the plot, useful for opening up space above the bars to put stars at postprocessing
...
def drawViolinPlot(data,dataLabels,outputPath,title,xLabel="",yLabel="",colors=['#2096BA','#351C4D', '#AB3E16','#849974', '#F7DFD4','#F5AB99'],rotation=0,yLim=[],scale=1.0): #shutter blue, nightfall, rust, fresh, macaron, tropical pink
    import matplotlib.pylab as plt
    import numpy as np
...
def drawBoxPlot(data,dataLabels,title,outputPath,xLabel="",yLabel="",colors=['#2096BA','#351C4D', '#AB3E16','#849974', '#F7DFD4','#F5AB99'],rotation=0,plotScatter=True,yLim=[],middleLine='mean', hlines=[],hlineColor="steelblue",hlineLabel="",legend=False,linesBetweenPlots=[],numLines=0,fontSize=16): #shutter blue, nightfall, rust, fresh, macaron, tropical pink
    import matplotlib.pylab as plt
    import numpy as np
    ...
    # horizontal line
    if hlines != []:
        ax.hlines(hlines,0,len(data),linestyle="dotted",color=hlineColor, label=hlineLabel)
    ...
    if(plotScatter==True):
        ...
        for i in range(len(data)):
            plt.scatter(xScattered[i],data[i],color=colors[i],alpha=0.5,s=5,edgecolors='dimgray',linewidth=0.3,zorder=1)

        # draw lines between a random sample of data points of 2 boxplots
        if linesBetweenPlots != []:
            x1,x2 = linesBetweenPlots
            xs = list(zip(xScattered[x1],xScattered[x2]))
            ys = list(zip(data[x1],data[x2]))
            outliers = sorted(range(len(data[x1])), key = lambda sub: data[x1][sub])[-20:]
            outliers += sorted(range(len(data[x2])), key = lambda sub: data[x2][sub])[-20:]
            outliers += sorted(range(len(data[x1])),reverse=True, key = lambda sub: data[x1][sub])[-20:]
            outliers += sorted(range(len(data[x2])),reverse=True, key = lambda sub: data[x2][sub])[-20:]
            sample = list(set(outliers+random.sample(range(0,len(xs)),numLines)))
            # for i in random.sample(range(0,len(xs)),numLines):
            for i in sample:
                # plt.plot(xs[i],ys[i],'crimson')
                if (ys[i][0]-ys[i][1])/(xs[i][0]-xs[i][1]) > 0: #slope is negative from left to right
                    plt.plot(xs[i],ys[i],'crimson',alpha=0.5,linewidth=0.5)
                else:
                    plt.plot(xs[i],ys[i],'dimgray',alpha=0.5,linewidth=0.5)

    if legend:     
        plt.legend(bbox_to_anchor=(1.0,0.4,0.3,0.3), loc="upper right",mode="expand",ncol=1,fancybox=True, framealpha=0.5,fontsize=12) #bbox_to_anchor=(x0,y0,width,height) bottomLeft of the screen is x,y=0,0
