/* 
 * File:   structureCoupling_MI.cpp
 * Author: yusuf
 *
 * Generated on February 22, 2018, 11:57 AM
 */
...
//Note: structural connectomes COULD have been edgeNormalized before calling this function
void StructureCoupling_MI::initializeAssignmentCostMatrix(float **assignmentCostMatrix, Graph &objects, Graph &labels)
{
   ...
   if(initializerParameters.assignmentCostMode.find("edges")!=std::string::npos)
   {
      ...
      srand((int)clock());
      //STEP 1:adjust structural connectivity matrices
      objects.initializeWeightedConnectivityMatrixFromEdgeFeatures(Edge::STRUCTURAL_CONNECTIVITY,dist1);
      labels.initializeWeightedConnectivityMatrixFromEdgeFeatures(Edge::STRUCTURAL_CONNECTIVITY,dist2);
      ...
      //Utility::saveMatrixToFile<float>("structuralConnectivity1.txt",dist1,numNodes,numNodes,'\t','\n',8);//DEBUG
      //Utility::saveMatrixToFile<float>("structuralConnectivity2.txt",dist2,numNodes,numNodes,'\t','\n',8);//DEBUG
...
void StructureCoupling_MI::initializeObjectsWeightMatrix(float **objectsWeightMatrix,Graph &objects)
{
...
//      spatial = 1- spatial;
      ...
      //w_pq = initializerParameters.getParameter(ParameterSet::SPATIAL_DISTANCE) * spatial + 
        w_pq = structural; //initializerParameters.getParameter(ParameterSet::STRUCTURAL_CONNECTIVITY) * structural +
...
void StructureCoupling_MI::initializeLabelsDistanceMatrix(float** labelsDistanceMatrix, Graph& labels)
{
...
//      spatial = (spatial-minSpatialDistance)/spatialDistanceRange;
      ...
      //due to high distance meaning negative correlation. (thus, -1 becomes 1, 1 becomes 0 in terms of distance)
//      functional = (1.0 - functional)/2.0;
      
      d_ab = structural; //initializerParameters.getParameter(ParameterSet::STRUCTURAL_CONNECTIVITY) * structural; 
