/* 
 * File:   functionCoupling_MI.cpp
 * Author: yusuf
 *
 * Generated on February 22, 2018, 11:59 AM
 */
...
//<editor-fold defaultstate="non-collapsed" desc=" Assignment Cost: initializeAssignmentCostMatrix()">
...
void FunctionCoupling_MI::initializeAssignmentCostMatrix(float **assignmentCostMatrix, Graph &objects, Graph &labels)
{
   //define a reasonable assignment cost function in here
   float assignmentCost;
   float ratio = objects.getNumNodes();
   ...
   if(initializerParameters.assignmentCostMode.find("edges")!=std::string::npos)
   {
      ...
      ////load structural
      objects.initializeWeightedConnectivityMatrixFromEdgeFeatures(Edge::FUNCTIONAL_CONNECTIVITY,dist1);
      ////load functional
      ...
      {//take the absolute value of the negative values only, discard the rest
         for(int i=0;i<numNodes;i++)
            for(int j=0;j<numNodes;j++)
               dist1[i][j] = dist1[i][j]>0 ? 0 : -dist1[i][j];
      }
      else if(initializerParameters.functionalConnectivity.compare("positive")==0)
         Utility::filterOutElementsOfMatrixLessThanThreshold<float>(dist1,0.0,0.0,numNodes,numNodes);
      else if(initializerParameters.functionalConnectivity.compare("full")!=0)
      {
         cerr<<"negative/positive/full is defined as <"<<initializerParameters.functionalConnectivity
                 <<"> for functional connectivity. Check the first parameter for -funcConn flag. Exiting...\n";
         exit(1);
      }
      
      //However, we will use either the positive or the negative correlations. Thus, we expect to get a half populated matrix
      //get rid of the negative (or positive) correlations
      if(initializerParameters.functionalConnectivity2.compare("negative")==0)
      {//take the absolute value of the negative values only, discard the rest
         for(int i=0;i<numNodes;i++)
            for(int j=0;j<numNodes;j++)
               dist2[i][j] = dist2[i][j]>0 ? 0 : -dist2[i][j];
      }
      else if(initializerParameters.functionalConnectivity2.compare("positive")==0)
         Utility::filterOutElementsOfMatrixLessThanThreshold<float>(dist2,0.0,0.0,numNodes,numNodes);
      else if(initializerParameters.functionalConnectivity2.compare("full")!=0)
      {
         cerr<<"negative/positive/full is defined as <"<<initializerParameters.functionalConnectivity2
                 <<"> for functional connectivity. Check the second parameter for -funcConn flag. Exiting...\n";
         exit(1);
      }
      ...
      //normalize the assignment cost values so that we know what range of values will go to the solver
      //it makes sense to normalize the assignment cost matrix as we already have normalized the structural and functional
      //connectomes before coming to here (in the experiment function this should have been done)
      //and the calculations being done in str-func coupling is always relative to subject
      Utility::normalizeMatrix(assignmentCostMatrix,numNodes,numNodes,0,1);
      //Utility::saveMatrixToFile<float>("normalizedAssignmentCostMatrix.txt",assignmentCostMatrix,numNodes,numNodes,'\t','\n',8);//DEBUG
...
void FunctionCoupling_MI::initializeObjectsWeightMatrix(float **objectsWeightMatrix,Graph &objects)
{
...
void FunctionCoupling_MI::initializeLabelsDistanceMatrix(float** labelsDistanceMatrix, Graph& labels)
{
